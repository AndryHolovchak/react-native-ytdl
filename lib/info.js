const urllib = require('./__REACT_NATIVE_YTDL_CUSTOM_MODULES__/urllib/url');
...
const TARGET_AUDIO_ITAG = 140;
exports.TARGET_AUDIO_ITAG = TARGET_AUDIO_ITAG;
exports.getAudioUrl = async (id, options = {}) => {
  const retryOptions = Object.assign({}, miniget.defaultOptions);
  options.requestOptions = Object.assign({}, options.requestOptions, {});
  options.requestOptions.headers = Object.assign(
    {},
    {
      //"CONSENT=YES+UA.uk+202008; SID=6QexvGIMTHmy2fu-fZbp0sc8PaqIrhRCwrDlBTZbWmcvN3ddcIAkQPrd2srzD6iPRTyVig.; APISID=xQGUbhaIFcP-_wmW/ALLTB6KF42NrmEqkK; SAPISID=ruauaAJ4iLgV1H-q/AMkbn4WJnBtVELyZ7; __Secure-3PAPISID=ruauaAJ4iLgV1H-q/AMkbn4WJnBtVELyZ7; _gcl_au=1.1.551888954.1611944562; PREF=f6=400&volume=100&tz=Europe.Kiev&f4=4000000; SIDCC=AJi4QfGwIrCA4LjHvX1qu3h5c-fDaJYkqogGBs_ceNFKici2HEchuG9_7iVLOm4podhe9enKGa0",
      //
  ...
  let info = await pipeline([id, options], validatePlaceholder, retryOptions, [
    getWatchHTMLPage,
  ]);
  let format = info.player_response.streamingData.adaptiveFormats.filter(
    (e) => e.itag == TARGET_AUDIO_ITAG,
  )[0];
  ...
  if (format) {
    ...
    funcs.push(sig.decipherFormats([format], html5player, options));
  }
...
const getWatchHTMLURL = (id, options) =>
  `${VIDEO_URL + id}&hl=${options.lang || 'en'}`;
const getWatchHTMLPageBody = (id, options) => {
  const url = getWatchHTMLURL(id, options);
  return exports.watchPageCache.getOrSet(url, () =>
    miniget(url, options.requestOptions).text(),
  );
};
...
const retryFunc = async (func, args, options) => {
  ...
  while (currentTry <= options.maxRetries) {
    try {
      ...
      if (
        err instanceof UnrecoverableError ||
        (err instanceof miniget.MinigetError && err.statusCode < 500) ||
...
const getWatchJSONURL = (id, options) =>
...
const getWatchJSONPage = async (id, options) => {
  ...
  reqOptions.headers = Object.assign(
    {
      ...
      'x-youtube-identity-token':
        exports.cookieCache.get(cookie || 'browser') || '',
    },
    reqOptions.headers,
  );
  ...
  const setIdentityToken = async (key, throwIfNotFound) => {
    ...
    reqOptions.headers['x-youtube-identity-token'] = await getIdentityToken(
      id,
      options,
      key,
      throwIfNotFound,
